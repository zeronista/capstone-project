        ...
        <tr class="hover:bg-gray-50 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-sm font-medium text-gray-900">${user.fullName || 'Chưa cập nhật'}</div>
                    </div>
                </div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
...
function showImportResults(result) {
    ...
    if (hasErrors && result.errors && result.errors.length > 0) {
        ...
        if (result.errors.length > 10) {
            errorListItems.innerHTML += `<li class="text-gray-500">... và ${result.errors.length - 10} lỗi khác</li>`;
        }
    } else {
...
function getRoleBadge(role) {
    const badges = {
        ...
        DOCTOR: '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">Bác sĩ</span>',
        RECEPTIONIST: '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Lễ tân</span>',
        PATIENT: '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">Bệnh nhân</span>'
    };
...
function getStatusBadge(isActive) {
    if (isActive) {
        return '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><span class="w-1.5 h-1.5 bg-green-500 rounded-full mr-1.5"></span>Hoạt động</span>';
    }
    return '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800"><span class="w-1.5 h-1.5 bg-red-500 rounded-full mr-1.5"></span>Vô hiệu</span>';
}
